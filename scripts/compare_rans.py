...
def plot_comparison():
    ...
    if metrics_file.exists():
        try:
            ...
            dp_blasius = df_m['Delta_P_Blasius_Pa'].iloc[0] if 'Delta_P_Blasius_Pa' in df_m.columns else "N/A"

            info_text = (
                f"$U_{{in}} = {cfg.velocity_inlet:.2f}$ m/s | $t_{{sim}} = {cfg.end_time:.1f}$ s\n"
                f"Malha: {cells} cél. | $\Delta P_{{Blasius}} = {dp_blasius}$ Pa\n"
                + "-"*40 + "\n"
            )

            for _, row in df_m.iterrows():
                model = row.get('Model', 'N/A')
                yplus = row.get('yPlus_Avg', 'N/A')
                dp = row.get('Delta_P_Pa', 'N/A')
                err = row.get('Error_Rel_Pct', 'N/A')

                info_text += (
                    f"[{model}]\n"
                    f" • $y^+_{{avg}}$: {yplus}\n"
                    f" • $\Delta P$: {dp} Pa (Erro: {err}%)\n\n"
                )
    ...
    plt.close()
    print(f"[GRÁFICO] Imagem atualizada com erro de Blasius em: {out_img}")
